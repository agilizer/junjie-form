		...
		success : function(data) {
			...
			if (data.length > 0) {
				if(targetSelectId=='catalogLevelThree'){
					$("#"+targetSelectId+"Div").html('<select class="form-control" name="catalog.id" style="" id="catalogLevelThree"></select>');
					$("#catalogLevelTwo").attr("name","catalogLevelTwo");
				}
				...
				for (var i = 0; i < data.length; i++) {
					...
					jQuery("#" + targetSelectId).append(
							"<option value='" + temp.id + "'>" + temp.text
									+ "</option>");
				}
				$("#" + targetSelectId).show()
				if (genNext == true) {
				...
				jQuery("#" + targetSelectId).empty();
				if(targetSelectId=='catalogLevelThree'){
					$("#"+targetSelectId+"Div").html('<input type="text"  placeholder="自定义分类 可以为空" id="catalogLevelThree"  class="form-control" name="customCatalogName">');
					$("#catalogLevelTwo").attr("name","catalog.id");
				}
			}
			var catalogJoinName = jQuery("#catalogLevelOne").find("option:selected").text(); 
			catalogJoinName = catalogJoinName +"->"+ jQuery("#catalogLevelTwo").find("option:selected").text(); 
...
$(document).ready(function() {
	...
	/**
	$('#catalogLevelOne').change(function() {
		changeSelect("catalogLevelOne", "catalogLevelTwo")
	})
	$('#catalogLevelTwo').change(function() {
		changeSelect("catalogLevelTwo", "catalogLevelThree")
	})
...
(function( $, window, undefined ) {
    $.danidemo = $.extend( {}, {
        ...
        addFile: function(id, i, file){
            ...
            '<img  class="demo-image-preview" id="img'+i+'"/>' +
                '<span class="demo-file-id">#' + i + '</span> - ' + file.name
                + ' <span class="demo-file-size" id="fileSize'+i+'">(' + $.danidemo.humanizeSize(file.size) +
    ...
    $('#drag-and-drop-zone').dmUploader({
       ...
       // extraData:extraData,
        ...
        onUploadSuccess: function (id, data) {
            ...
            html='<button class="btn btn-primary btn-xs" type="button" id="imageSet'+data.fileId+'" onclick="setMainPhoto(this)" data-id="'
...
function delPhoto(btn){
    ...
    if(confirm("确定删除?")){
    	...
    	var data={fileId:id,mainPhoto:'false'}
...
function setMainPhoto(btn){
	var obj = $(btn)
	var oldFileId = $("#mainPhotoId").val()
	$("#imageSet"+oldFileId).html("设置首图");
...
function addToCarousel(btn){
	...
	var html='<input type="hidden" name="imagesCarousel.id" value="'+fileId+'" id="imagesCarouselId'+fileId+'"/>'
    $("#catalogJoinName").after(html)
    obj.html("轮播图片中移除");
	obj.attr("onclick","removeToCarousel(this)");
}

function removeToCarousel(btn){
	var obj = $(btn)
	var fileId = obj.attr('data-id')
	$("#imagesCarouselId"+fileId).remove()
	obj.html("添加到轮播图片");
